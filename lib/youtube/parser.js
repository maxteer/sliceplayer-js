...
var _formats = require('../utils/formats'); var _formats2 = _interopRequireDefault(_formats);
...
const ASSETS_REGEX = /"assets":.+?"js":\s*"([^"]+)"/;
 ...
 async function getPlayerConfig(url, attempts = 2) {
  let reason;
  let playerConfig;
  let matcher;
  while (attempts-- > 0 && !playerConfig) {
    try {
      ...
      const json = JSON.parse(_extractor2.default.extractYtPlayerConfig(html));
 ...
 function getVideoDetails(config) {
 ...
 async function parseFormats(config) {
  const playerResponse = config.args['player_response'];
  if (!playerResponse.streamingData) {
 ...
 function getPlaylistDetails(id, initialData) {
 ...
 async function getPlaylistVideos(initialData) {
  let content = null;
  try {
    content =
...
const parseFormat = async (json, config, isAdaptive) => {
  if (json.signatureCipher) {
    ...
    if (!preSigned) {
      ...
      const cipher = await _cipher.createCipher.call(void 0, jsUrl);
  ...
  const format = Object.assign({}, _formats2.default[json.itag]);
  format.url = json.url;
